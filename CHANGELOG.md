...
- **`CalloutBuilder` is extensible** — the class is `virtual`, `withEndpoint`, `withMethod`, `withBody`, `withBlobBody`, and `getHttpResponse` are overridable, and `getEndpoint()`, `getHeaders()`, `getQueryParameters()` expose request state that previously had no read path. Every response variant funnels through `getHttpResponse()`, so a subclass overriding it observes every callout — this is how `GuardedCalloutBuilder` enforces its guardrails ([CalloutBuilder.cls](force-app/main/calloutBuilder/classes/CalloutBuilder.cls))
